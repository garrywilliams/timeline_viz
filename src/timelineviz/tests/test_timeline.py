...
def sample_df():
    return pd.DataFrame(
        {
            "order_id": ["123", "124"],
            "created_at": [datetime(2024, 1, 1, 10, 0), datetime(2024, 1, 2, 11, 0)],
            "updated_at": [datetime(2024, 1, 2, 15, 30), datetime(2024, 1, 3, 16, 30)],
            "completed_at": [datetime(2024, 1, 3, 9, 45), datetime(2024, 1, 4, 10, 45)],
        }
    )
...
def test_plot_multiple_timelines_errors():
    ...
    with pytest.raises(FileNotFoundError):
        plot_multiple_timelines(data="nonexistent.csv", timestamp_columns=["col"], show_plots=False)

    # Test invalid DataFrame
    with pytest.raises(ValueError):  # Changed to expect ValueError
        plot_multiple_timelines(data=None, timestamp_columns=["col"], show_plots=False)

    # Test empty DataFrame
    df = pd.DataFrame()
    result = plot_multiple_timelines(data=df, timestamp_columns=["col"], show_plots=False)
    assert result == []
...
def test_plot_timeline_customization():
    data = pd.Series({"event1": pd.Timestamp("2024-01-01"), "event2": pd.Timestamp("2024-01-02")})

    # Test custom figure size
    fig, ax = plot_timeline(
        data, timestamp_columns=["event1", "event2"], figsize=(20, 8), show_plot=False
    )
    ...
    fig, ax = plot_timeline(
        data, timestamp_columns=["event1", "event2"], point_size=15, show_plot=False
    )
    plt.close(fig)
...
def test_plot_multiple_timelines_sorting():
    ...
    processed = plot_multiple_timelines(
        data=df, timestamp_columns=["first", "middle", "last"], id_column="id", show_plots=False
    )
...
def test_plot_multiple_timelines_validation():
    ...
    df = pd.DataFrame({"id": [1], "valid_date": ["2024-01-01"], "invalid_date": ["not a date"]})
...
def test_plot_timeline_advanced_features():
    ...
    label_mappings = {"t1": "Start", "t2": "Process", "t3": "Review", "t4": "Complete"}
...
def test_plot_timeline_error_handling():
    ...
    data = pd.Series({"valid_date": pd.Timestamp("2024-01-01"), "invalid_date": "not a date"})
    ...
    fig, ax = plot_timeline(data, timestamp_columns=["invalid1", "invalid2"], show_plot=False)
    assert fig is None
    assert ax is None
...
def test_plot_timeline_color_schemes():
    ...
    fig, ax = plot_timeline(data, timestamp_columns=["event1", "event2"], show_plot=False)
    plt.close(fig)
    ...
    fig, ax = plot_timeline(
        data, timestamp_columns=["event1", "event2"], color_scheme=custom_colors, show_plot=False
    )
    plt.close(fig)
...
def test_plot_timeline_output_formats():
    ...
    try:
        fig, ax = plot_timeline(
            data, timestamp_columns=["event1", "event2"], output_file=png_file, show_plot=False
        )
...
def test_clean_column_name_comprehensive():
    ...
    assert (
        clean_column_name("test_column_at_utc", ["_utc", "_at"]) == "Test Column At"
...
def test_plot_multiple_timelines_edge_cases(tmp_path):
    ...
    with pytest.raises(OSError):
        plot_multiple_timelines(
            data=df, timestamp_columns=["ts"], output_dir="/nonexistent/dir", show_plots=False
        )
...
def test_plot_multiple_timelines_invalid_data():
    ...
    with pytest.raises(KeyError):
        plot_multiple_timelines(
            data=df, timestamp_columns=["ts"], id_column="nonexistent", show_plots=False
        )
...
def test_plot_multiple_timelines_with_entity_name():
    ...
    result = plot_multiple_timelines(
        data=df, timestamp_columns=["ts"], entity_name="Patient", show_plots=False
    )
    assert len(result) > 0
...
def test_plot_sorted_events_no_clusters(monkeypatch):
    ...
    monkeypatch.setattr(timeline_module, "find_clusters", lambda *_args, **_kwargs: ([], []))
    fig, ax = _plot_sorted_events(
...
def test_plot_multiple_timelines_invalid_threshold():
    ...
    with pytest.raises(ValueError):
        plot_multiple_timelines(df, ["ts"], threshold_days=-1, show_plots=False)
...
def test_plot_multiple_timelines_combines_specified_and_detected_columns(monkeypatch):
    ...
    df = pd.DataFrame({"id": [1], "ts1": ["2024-01-01"], "ts2": ["2024-01-02"]})
    seen = {}

    def fake_plot_timeline(data, timestamp_columns, **kwargs):
        seen["timestamp_columns"] = timestamp_columns
        return plt.figure(), []

    monkeypatch.setattr(timeline_module, "detect_timestamp_columns", lambda _df: ["ts2"])
    monkeypatch.setattr(timeline_module, "plot_timeline", fake_plot_timeline)

    processed = plot_multiple_timelines(
        df,
        timestamp_columns=["ts1"],
    ...
    assert processed == ["1"]
    assert set(seen["timestamp_columns"]) == {"ts1", "ts2"}


def test_plot_multiple_timelines_without_columns_or_detection():
    df = pd.DataFrame({"name": ["test"]})
    assert plot_multiple_timelines(df, show_plots=False) == []


def test_plot_multiple_timelines_skips_nan_identifier(capsys):
    df = pd.DataFrame({"id": [float("nan")], "ts": ["2024-01-01"]})
    processed = plot_multiple_timelines(
        df, timestamp_columns=["ts"], id_column="id", show_plots=False
    )
    assert processed == []
    assert "No data found for entity nan" in capsys.readouterr().out


def test_plot_timeline_empty_clusters():
    # Test with data that results in empty clusters
    data = pd.Series({"ts1": pd.Timestamp("2024-01-01"), "ts2": pd.Timestamp("2024-06-01")})
    fig, ax = plot_timeline(
        data,
        timestamp_columns=["ts1", "ts2"],
...
def test_plot_multiple_timelines_output_handling(tmp_path):
    ...
    result = plot_multiple_timelines(
        df, timestamp_columns=["ts"], output_dir=str(output_dir), show_plots=False
    )
    assert len(result) > 0
    assert output_dir.exists()
